...
function isHistoricalSegment(segment: RouteSegment): boolean {
  return segment.kind === "chronology"
...
export function resolveJourneyLeg(
  ...
  const directHistorical = historicalSegments.find(
    (segment) => isHistoricalSegment(segment)
      && segment.from_id === fromScene.id
      && segment.to_id === toScene.id,
  );
  ...
  const reverseHistorical = historicalSegments.find(
    (segment) => isHistoricalSegment(segment)
      && segment.from_id === toScene.id
      && segment.to_id === fromScene.id,
  );
