...
.chip{display:inline-flex; align-items:center; gap:4px; font-size:13px;
  ...
  border:1px solid transparent; background:#fff; border-left-width:4px;}
...
.mini-chip{font-size:11.5px; padding:0 6px; border-radius:4px; border-left:3px solid;
  background:#fff; font-family:KaiTi,STKaiti,serif;}
...
.hit{padding:0 2px; border-radius:4px; border-bottom:2px solid;}
...
.card,.star-block,details.method{background:rgba(251,252,250,.91); backdrop-filter:blur(1px);}
.chip,.mini-chip,.poem-box,.pcard,.border-col{background:rgba(255,255,255,.89);}
</style>
</head>
<body>
  ...
  D.poets.forEach(function(p, idx){
    ...
    p.audible.forEach(function(a){
      ...
      a.hits.forEach(function(hn){
        var mc = el("span", "mini-chip", hn[0] + (hn[1] > 1 ? "×" + hn[1] : ""));
