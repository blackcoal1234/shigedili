...
POET_CONFIG = {
    "李白": {"key": "libai", "color": "#426f94", "dynasty": "唐"},
    "杜甫": {"key": "dufu", "color": "#7a5c3d", "dynasty": "唐"},
    "白居易": {"key": "baijuyi", "color": "#26786e", "dynasty": "唐"},
    "苏轼": {"key": "sushi", "color": "#b64b3f", "dynasty": "宋"},
    "陆游": {"key": "luyou", "color": "#8a3b2f", "dynasty": "宋"},
    "李清照": {"key": "liqingzhao", "color": "#9c5d8f", "dynasty": "宋"},
}
...
.map-pane{position:relative;min-width:0;background:#edf0e8}.map{height:610px;width:100%}.map-note{position:absolute;left:14px;bottom:12px;right:14px;background:#fffffff0;border:1px solid var(--line);padding:8px 10px;font-size:12px;color:var(--muted);pointer-events:none}
...
@media(max-width:820px){.wrap{width:min(100% - 18px,1380px)}.toolbar{grid-template-columns:1fr}.progress{grid-column:auto}.actions{justify-content:flex-start}.stage{grid-template-columns:1fr}.map{height:390px}.stage{min-height:0}.story-pane{border-left:0;border-top:1px solid var(--line);padding:17px}.collision{grid-template-columns:1fr}.pipeline{grid-template-columns:1fr 1fr}.hero{padding-top:22px}.section{padding:20px 0}}
@media(max-width:430px){.actions .btn{flex:1 1 calc(50% - 5px)}.seg{width:100%}.seg button{flex:1}.story-pick{align-items:flex-start;flex-direction:column}.map{height:350px}.pipeline{grid-template-columns:1fr}.poem{font-size:18px}.scene-head h3{font-size:21px}footer a{display:inline-block;margin:4px 5px}}
</style>
</head>
<body>
<header class="hero"><div class="wrap">
  ...
  <h1>诗篇有年，行旅自动成片</h1>
  <p>系统合并诗篇系年、审核记录与候选史料，自动排序年份区间、落图并生成镜头。精确年份与约年均保留并明确标注；争议系年不进入年份碰撞。观看者只控制节奏，不手工编排路线。</p>
  <div class="kpis"><span class="kpi" id="kCorpus"></span><span class="kpi" id="kStories"></span><span class="kpi" id="kScenes"></span><span class="kpi" id="kCollision"></span></div>
</div></header>
...
<section class="section">
  ...
  <div class="stage">
    <div class="map-pane"><div class="map" id="routeMap"></div><div class="map-note">只有年份可严格排序且坐标完整的相邻诗篇才连线；断线不是数据错误。地图可缩放，点可悬停查看史料。</div></div>
...
<section class="section">
  ...
  <div class="table-scroll"><table class="route-table"><thead><tr><th>序</th><th>年份</th><th>地点</th><th>诗篇</th><th>等级</th><th>来源</th></tr></thead><tbody id="routeRows"></tbody></table></div>
</section>
...
(function(){
"use strict";
var D=window.AUTO_STORY_DATA, stories=D.stories, storyIndex=0, stepIndex=0, collisionIndex=0, auto=false, timer=0, chart=null;
...
function renderMap(){
 var story=currentStory(), guides=story.segments.map(function(seg){return {coords:seg.coords};}), traveled=story.segments.filter(function(seg){return seg.to_index<=stepIndex;}).map(function(seg){return {coords:seg.coords};});
 if(!chart){chart=echarts.init(document.getElementById("routeMap"));window.addEventListener("resize",function(){chart.resize();});}
 chart.setOption({animationDurationUpdate:700,tooltip:{trigger:"item",confine:true,formatter:function(p){var s=p.data&&p.data.scene;if(!s){return p.name||"";}return "<b>"+esc(s.place_historical)+"</b> · "+s.year_label+"<br>"+esc(s.poet)+"《"+esc(s.poem_title)+"》<br>"+esc(s.source_status+" "+s.source_grade+"级");}},geo:{map:"china",roam:true,zoom:2.15,center:[108,32],itemStyle:{areaColor:"#edf0e8",borderColor:"#c3cabe",borderWidth:.7},emphasis:{itemStyle:{areaColor:"#e6ebe4"}}},series:[{id:"guide",type:"lines",coordinateSystem:"geo",silent:true,lineStyle:{color:"#aeb7ad",width:1.2,type:"dashed"},data:guides},{id:"traveled",type:"lines",coordinateSystem:"geo",silent:true,lineStyle:{color:story.color,width:3,cap:"round"},data:traveled},{id:"nodes",type:"effectScatter",coordinateSystem:"geo",showEffectOn:"emphasis",rippleEffect:{scale:2.7,brushType:"stroke"},data:routeData(story)}]},true);
}
function renderScene(resetClock){
 ...
 document.getElementById("sceneBadges").innerHTML='<span class="badge grade-'+s.source_grade.toLowerCase()+'">'+esc(s.source_status+" "+s.source_grade+"级")+'</span><span class="badge">'+esc(s.year_precision_display)+'</span><span class="badge">场景 '+(stepIndex+1)+'/'+story.scene_count+'</span>'+(s.scene_image?'<span class="badge ai">AI场景重建</span>':'');
...
function renderMethod(){var m=D.method;document.getElementById("methodBody").innerHTML='<ol><li>'+esc(m.route_rule)+'</li><li>'+esc(m.line_rule)+'</li><li>'+esc(m.poem_rule)+'</li><li>'+esc(m.source_rule)+'</li><li>'+esc(m.collision_rule)+'</li><li>'+esc(m.image_rule)+'</li></ol><p>动画引擎：'+esc(D.meta.engine)+'。模型图像属于气氛层，史料结论只来自本地审核数据。</p>';}
...
initMeta();renderCollision();renderUnresolved();renderMethod();renderAll(true);
})();
</script>
</body></html>'''
