...
- 88 位唐宋精选名家。
- 状态层：80,893 首名家全作品，用于人物状态、文本画像、意象、声音与情感等聚合分析。
- canonical 展示证据层：20,437 首规范诗篇，用于原文展示、编年地理、证据回链、译注与赏析；稳定 ID 精确关联，不按作者与同题标题猜测合并。
- 六位核心诗人：李白、杜甫、白居易、苏轼、陆游、李清照；状态分析覆盖六人 19,536 首全作品，不再使用按人定额的截断子样本。
- 参赛版由 29 号导航统一进入 30–44 号，共 15 个展项；44 号为可深链的赏析诗页入口。
...
仓库仍保留网络采集、关系建模和数据库教学链路；正式发布的文本状态统计以 80,893 首全作品为准，诗篇展示与事实证据以 20,437 首 canonical 规范层为准。
...
## 4. 目录结构
    ...
    │   ├── build_poem_page_data.py
    │   ├── check_poem_page_data.py
    │   ├── build_rich_guides_llm.py
    │   └── serve_output.py
    ├── output/
    │   ├── assets/
    │   ├── index.html
...
核心样本扩容至 155 首后，其中 118 首有采集尝试状态（李白新增 35 首暂未进入背景采集；另有 2 首因正文修订暂未按正文哈希匹配到历史采集状态）；88 位诗人均有身份处理状态。41 条已批准记录目前只完成作年作地，尚未同时满足“120–220 字背景故事、逐句自有译文、至少 2 条注释和 1 条赏析要点”，因此完整版进度必须如实记为 `0/60`，不能用模型批量生成后直接宣称验收完成。2026-08 起另设「助手续写译注赏析」层（`data/assistant_rich_backgrounds/`，七批共 69 首：36 首可挂库 approved 名篇含琵琶行、长恨歌、赤壁赋三大长篇，另 33 首语料名篇扩展至王维、王昌龄、杜牧、李商隐、辛弃疾、李煜等二十四家）：助手撰写背景故事（按三层事实锚定措辞，语料异文如实标注）、逐句译文、注释与赏析要点，在赏析诗页以独立区块呈现并标注「助手撰写 · 非人工考据」与批次号；人工复核通过前不计入 `0/60` 验收口径，复核后转入 `verified_poem_backgrounds.jsonl`。全量规模化走 `tools/build_rich_guides_llm.py`（OpenAI 兼容接口，`AGENT_LLM_*` 环境变量），产出入 `data/llm_rich_backgrounds/`、徽章「模型生成」，与手写层同套逐字质量门，手写层优先。按需生成另有两种入口：命令行 `python tools/build_rich_guides_llm.py --poem-id <id>`（单首，留档 `batch_auto_001.json`）；Agent API `GET/POST /knowledge/rich-guide`（查询/生成，44_诗页对无译注的诗显示「在线生成」按钮，无 Key 时明确降级提示、离线浏览不受影响）。从静态站点访问 Agent API 需将其来源加入 `AGENT_ALLOWED_ORIGINS` 环境变量。

赏析模型的外部依据登记在 `data/reviewed/poem_appreciation_references.json`，并以正文 `body_hash` 精确绑定。维基文库只用于原文/版本核对，搜韵用于经转述审核的词义、典故、格律与出处，中国哲学书电子化计划用于古籍原典追溯；古文岛（原古诗文网）只作人工软对照，禁止复制其现代译文和赏析。执行 `python tools/build_rich_guides_llm.py --require-reference --limit 10` 可只处理已有审核摘要的诗；每个故事、逐句译注和赏析要点都必须携带 `P0`、可选 `F0` 或当前诗 `R...` 证据 ID。登记表中的 `approved` 仅表示该短摘要可进入提示词，不等于人工发布终审。
...
使用现有双层语料（状态层 80,893 首、canonical 展示证据层 20,437 首）、导入 MySQL 并生成全部主题页面：
...
| output/29_参赛导航.html | 参赛版 30–44 号共 15 个展项的总导航 |
...
| output/39_诗人自述生命卷.html | 88 位诗人的第一视角生命叙事与 canonical 证据回链 |
| output/40_山河证道.html | 基于审核作地证据的诗词地理闯关 |
| output/41_意象地理.html | 地域与意象过表征倍率及原句证据 |
| output/42_被想象的地方.html | 亲历书写与异地想象的核验级对照 |
| output/43_飞花令加行.html | 地名、意象与古今地名三题型番外 |
| output/44_诗页.html | canonical 展示证据层 20,437 首的赏析入口：一首诗一页，支持深链、译注赏析、审核背景与在线生成降级 |
...
`output/manifest.json`，因此由 29 号导航进入的 30–44 号 15 个页面仍可作为稳定离线展项使用。实验入口把
...
- Python 语法。
- 80,893 首状态层全作品与 20,437 首 canonical 展示证据层；六位核心诗人的状态统计使用全作品口径。
- 88 位身份状态、候选审核状态、证据短引、赏析诗页数据与发布完整度。
- 精神意象词典自检（簇合法、尺度 1–5、无重复、尺度依据齐全）、李白候选编年（期2–5 每期至少 3 条、来源与等级合规）与精神地形图页面（echarts 初始化、推定样式）。
- 行旅节点字段、坐标、来源与等级。
- 创作背景与现有作品精确匹配。
- 同意象计算口径和证据诗句。
- 核心 HTML、参赛版 30–44 号 15 个展项及 29 号导航、离线脚本依赖、入口链接和 manifest 哈希。
