...
function WorkbenchBody({
  ...
  useEffect(() => {
    const synchronizeDeepLink = () => {
      const params = new URL(window.location.href).searchParams;
      const poemId = params.get("poem");
      const lineId = params.get("line");
      setKnowledgePoemId(poemId);
      setKnowledgeLineId(poemId ? lineId : null);
      setKnowledgeOpen(params.get("knowledge") === "1" || Boolean(poemId));
    };
    synchronizeDeepLink();
    window.addEventListener("popstate", synchronizeDeepLink);
    return () => window.removeEventListener("popstate", synchronizeDeepLink);
  }, []);

  const updateKnowledgeSelection = useCallback((
    poemId: string | null,
    lineId: string | null,
  ) => {
    setKnowledgePoemId(poemId);
    ...
    url.searchParams.set("knowledge", "1");
    if (poemId) url.searchParams.set("poem", poemId);
    else url.searchParams.delete("poem");
    if (poemId && lineId) url.searchParams.set("line", lineId);
  ...
  return (
    <main className="workbench-shell">
      ...
      <KnowledgeExplorer
        open={knowledgeOpen}
        initialPoemId={knowledgePoemId}
        initialLineId={knowledgeLineId}
